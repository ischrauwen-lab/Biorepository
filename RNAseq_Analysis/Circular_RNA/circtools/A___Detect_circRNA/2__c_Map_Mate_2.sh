...
#$ -l h_rt=24:00:00
#$ -l h_vmem=25G
#$ -o /path/to/Mapping_Mate_2-$JOB_ID.out
#$ -e /path/to/Mapping_Mate_2-$JOB_ID.err
#$ -N Mapping_Mate_2
#$ -j y
#$ -q queue
...
for dir in "${Input_Directory}"/*; do
    ...
    #Output Directory
    Output_Directory="${sample_dir}/mate2"
    ...
    for reverse_file in "${dir}"/*_Unmapped.out.mate2.gz; do
        # Extract the file name without extension
        file_name=$(basename "${reverse_file}" _Unmapped.out.mate2.gz)

        echo "Reverse File: ${reverse_file}"
        echo "Output Directory: ${Output_Directory}"

        # Create a unique temporary directory for this sample
        TMPDIR="${sample_dir}/${file_name}_STARtmp_mate2"
